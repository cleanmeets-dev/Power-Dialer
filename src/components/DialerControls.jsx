...
export default function DialerControls({
  ...
  const handleStart = async () => {
    ...
    try {
      await startDialing(campaignId, isAgentMode ? agentId : agentId || null);
      setIsDialing(true);
  ...
  const handleStop = async () => {
    try {
      await stopDialing(campaignId, isAgentMode ? agentId : agentId || null);
      setIsDialing(false);
    ...
    <div className="bg-linear-to-br from-slate-100 to-slate-50 dark:from-slate-800 dark:to-slate-700 rounded-lg shadow-2xl dark:shadow-slate-900/30 p-6 border border-slate-200 dark:border-slate-700">
      ...
      <div className="flex gap-4 mb-4 flex-wrap">
        ...
        >
          ...
          {isAgentMode ? "Start Auto Dialer" : "Start Power Dialer"}
        </button>
        <button
          onClick={handleStop}
        ...
        >
          ...
          {isAgentMode ? "Stop Sequence" : "Stop Power Dialer"}
        </button>
      </div>
      ...
      {isDialing && (
        <div className="p-4 bg-blue-500/20 border border-blue-500/50 rounded flex items-center gap-3">
          ...
          <span className="text-blue-400 font-semibold">
            {isAgentMode
              ? "Auto dialer is running from the backend orchestrator..."
