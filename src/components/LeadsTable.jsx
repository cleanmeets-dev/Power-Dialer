...
export default function LeadsTable({ showNotification, activeCalls = [] }) {
  ...
  const canExportLeads = user?.role === "admin" || user?.role === "manager";
            ...
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-700/50 ">
                ...
                {tableColumns.map((col) => (
                    <th key={col.key} className={`${col.width || ""} text-left py-3 px-2 text-cyan-700 dark:text-cyan-400 font-semibold`}>
                    {col.label}
                  </th>
                ))}
            ...
            <tbody>
              {leads.map((lead) => (
                ...
                >
                  ...
                  {tableColumns.map((col) => (
                    <td key={`${lead._id}-${col.key}`} className={`${col.width || ""} py-3 px-2 text-slate-900 dark:text-slate-200 align-top`}>
