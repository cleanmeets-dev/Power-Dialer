...
function getSidebarItems(role) {
	if (role === 'manager') {
		return [
			...
			{ id: 'agents', label: 'Agents', icon: Users, path: '/manager/agents' },
		];
	}
	...
	return [
		...
		{ id: 'my-availability', label: 'My Status', icon: Clock, path: '/agent/my-availability' },
	];
}
	...
	return (
		<>
			<aside className="hidden md:block w-72 shrink-0">
				<div className="sticky top-24 rounded-2xl border border-slate-700 bg-slate-800/80 backdrop-blur p-4">
					<p className="text-xs uppercase tracking-wider text-slate-400 mb-3">Navigation</p>
			...
			{isOpen && (
				<div className="md:hidden fixed inset-0 z-40">
					...
					/>
					<aside className="absolute left-0 top-0 h-full w-72 bg-slate-900 border-r border-slate-700 p-4">
						<p className="text-xs uppercase tracking-wider text-slate-400 mb-3">Navigation</p>
						<nav className="space-y-2">
							{navItems.map(({ id, label, icon: Icon, path }) => (
								<NavLink
