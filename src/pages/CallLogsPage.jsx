...
export default function CallLogsPage() {
  ...
  const getStatusColor = (outcome) => {
    switch (outcome) {
      ...
      default:
        return 'bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-400';
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-linear-to-r from-slate-100 to-slate-50 dark:from-slate-800 dark:to-slate-700 rounded-lg shadow-2xl dark:shadow-slate-900/30 p-6 border border-slate-200 dark:border-slate-700">
        <h1 className="text-3xl font-bold text-slate-900 dark:text-primary-400">Call Logs</h1>
        <p className="text-slate-600 dark:text-slate-400 mt-2">Review your calling history</p>
      </div>
      ...
      {selectedCampaignId && (
        <div className="bg-linear-to-br from-slate-100 to-slate-50 dark:from-slate-800 dark:to-slate-700 rounded-lg shadow-2xl dark:shadow-slate-900/30 border border-slate-200 dark:border-slate-700 overflow-hidden">
          {/* Error Alert */}
          {error && (
          ...
          {isLoading && callLogs.length === 0 && (
            <div className="p-8 text-center">
              ...
              <p className="text-slate-600 dark:text-slate-400">Loading call logs...</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-6 border-b border-slate-200 dark:border-slate-700">
            <div className="bg-slate-100 dark:bg-slate-700/50 p-4 rounded-lg">
              ...
              <p className="text-2xl font-bold text-cyan-700 dark:text-cyan-400">{callLogs.length}</p>
            </div>
            <div className="bg-slate-100 dark:bg-slate-700/50 p-4 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                <MessageSquare className="w-4 h-4 text-emerald-400" />
                <span className="text-slate-600 dark:text-slate-400 text-sm">Answered</span>
              </div>
            ...
            <div className="bg-slate-100 dark:bg-slate-700/50 p-4 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-slate-600 dark:text-slate-400 text-sm">Avg Duration</span>
              </div>
          ...
          <div className="divide-y divide-slate-200 dark:divide-slate-700">
            {callLogs.map(log => (
              <div key={log._id} className="p-4 hover:bg-slate-100 dark:hover:bg-slate-700/30 transition">
                <div
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3">
                      <span className="text-slate-900 dark:text-slate-200 font-semibold truncate">{log.lead?.phoneNumber || 'Unknown'}</span>
                ...
                {expandedLogId === log._id && (
                  <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700 space-y-3 text-sm">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <span className="text-slate-600 dark:text-slate-400 block text-xs uppercase tracking-wide">Phone</span>
                        <span className="text-slate-900 dark:text-slate-200 font-semibold">{log.lead?.phoneNumber || 'N/A'}</span>
                      </div>
                      <div>
                        <span className="text-slate-600 dark:text-slate-400 block text-xs uppercase tracking-wide">Outcome</span>
                        <span className="text-slate-900 dark:text-slate-200 font-semibold capitalize">{log.outcome || 'N/A'}</span>
                      </div>
                      <div>
                        <span className="text-slate-600 dark:text-slate-400 block text-xs uppercase tracking-wide">Duration</span>
                        <span className="text-slate-900 dark:text-slate-200 font-semibold">{formatDuration(log.duration)}</span>
                      </div>
                      <div>
                        <span className="text-slate-600 dark:text-slate-400 block text-xs uppercase tracking-wide">Date</span>
                        <span className="text-slate-900 dark:text-slate-200 font-semibold">{new Date(log.createdAt).toLocaleDateString()}</span>
                      </div>
                    </div>
                    
                    {log.recordingUrl && (
                      <div className="border-t border-slate-300 dark:border-slate-600 pt-3">
                        <span className="text-slate-600 dark:text-slate-400 block text-xs uppercase tracking-wide mb-2">Call Recording</span>
                    ...
                    {log.agentNotes && (
                      <div className="border-t border-slate-300 dark:border-slate-600 pt-3">
                        <span className="text-slate-600 dark:text-slate-400 block text-xs uppercase tracking-wide mb-1">Agent Notes</span>
                        <span className="text-slate-900 dark:text-slate-200">{log.agentNotes}</span>
                      </div>
                    )}
                    <div className="border-t border-slate-300 dark:border-slate-600 pt-3 grid grid-cols-2 gap-3">
                      <div>
                        <span className="text-slate-600 dark:text-slate-400 block text-xs uppercase tracking-wide mb-1">Call Quality</span>
                        <span className="text-slate-900 dark:text-slate-200 font-semibold">{log.callQuality || 'Not Rated'}</span>
                      </div>
                      <div>
                        <span className="text-slate-600 dark:text-slate-400 block text-xs uppercase tracking-wide mb-1">Sentiment</span>
                        <span className="text-slate-900 dark:text-slate-200 font-semibold">{log.sentiment || 'Not Analyzed'}</span>
                      </div>
                    </div>
                  </div>
                )}
          ...
          {!isLoading && callLogs.length === 0 && (
            <div className="text-center py-12">
              <Phone className="w-12 h-12 text-slate-500 mx-auto mb-3" />
              <p className="text-slate-600 dark:text-slate-400">No call logs yet</p>
      ...
      {!selectedCampaignId && (
        <div className="text-center py-12 bg-linear-to-br from-slate-100 to-slate-50 dark:from-slate-800 dark:to-slate-700 rounded-lg border border-slate-200 dark:border-slate-700">
          <p className="text-slate-600 dark:text-slate-400">Select a campaign to view call logs</p>
        </div>
      )}
    </div>
  );
